...
class AiProvider extends ChangeNotifier {
  ...
  /// Отправить вопрос AI-ассистенту (через RAG HF Space или GLM fallback)
  Future<void> sendMessage(String content) async {
    ...
    try {
      ...
      if (ragAnswer != null) {
        ...
        // Fallback на прямой GLM с RAG контекстом
  ...
  /// Запрос к RAG через HF Space Gradio API
  Future<String?> _askRagViaHfSpace(String query) async {
    try {
      final response = await http.post(
        Uri.parse('${ApiConfig.hfSpaceUrl}/call/rag_search'),
      ...
      if (response.statusCode == 200) {
        ...
        if (eventId != null) {
          final resultResponse = await http.get(
            Uri.parse('${ApiConfig.hfSpaceUrl}/call/rag_search/$eventId'),
          ).timeout(const Duration(seconds: 90));
